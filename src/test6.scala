...
object test6 {
  private def createChart(var0: XYDataset): JFreeChart = {
    val var1: JFreeChart = ChartFactory.createTimeSeriesChart("Legal & General Unit Trust Prices", "Date", "Price Per Unit", var0, true, true, false)
    ...
    val var3: XYItemRenderer = var2.getRenderer
    if (var3.isInstanceOf[XYLineAndShapeRenderer]) {
      val var4: XYLineAndShapeRenderer = var3.asInstanceOf[XYLineAndShapeRenderer]
      var4.setBaseShapesVisible(false)
    }
    val var5: DateAxis = var2.getDomainAxis.asInstanceOf[DateAxis]
  ...
  private def createDataset: XYDataset = {
    ...
    val var1: TimeSeries = new TimeSeries("L&G UK Index Trust")
  ...
  def main(var0: Array[String]) {
    val var1: test6 = new test6("Time Series Demo 1")
    var1.pack
...
class test6(val var1: String) extends ApplicationFrame(var1) {
  val var2: JPanel = TimeSeriesDemo1.createDemoPanel
